...
{
	public static class LootTableCollectionProvider
	{

		/// <summary>
		/// Builds the table from the database using the passed table Id.
		/// </summary>
		public static LootTableCollection LoadTable(string tableId)
		{
			var ntab = new LootTableCollection(tableId);
			// Get the data
			DBRecordCollection record = Database.DB.GetRecordFromFile(ntab.tableId);
			if (record == null)
			{
				return ntab;
			}
