...
namespace TQVaultAE.DAL
{
	...
	public static class ItemProvider
	{
		...
		public static string ToString(Item itm, bool basicInfoOnly = false, bool relicInfoOnly = false, bool secondRelic = false)
		{
		...
		public static string GetAttributes(Item itm, bool filtering)
		{
			...
			// Add the item name
			string itemName = Database.MakeSafeForHtml(ToString(itm, true, false));
			Color color = itm.GetColorTag(itemName);
			...
			if (itm.baseItemInfo != null)
			{
				if (itm.IsRelic)
				{
					...
					if (!itm.IsRelicComplete)
					{
						...
						str = Format(formatSpec, type, itm.Number, itm.baseItemInfo.CompletedRelicLevel);
					}
					else
					{
						string tag = "tagRelicComplete";
					...
					Color color1 = itm.GetColorTag(str);
				...
				else if (itm.IsArtifact)
				{
					...
					results.Add(string.Format(CultureInfo.CurrentCulture, "<font color={0}>{1}</font>", Database.HtmlColor(Item.GetColor(ItemStyle.Broken)), artifactClass));
				}
				...
				if (itm.IsFormulae)
				{
					...
					results.Add(string.Format(CultureInfo.CurrentCulture, "<font color={0}>{1}</font><br>", Database.HtmlColor(Item.GetColor(ItemStyle.Mundane)), recipe));
					...
					str = Format(formatSpec, (object)0, 3);
					str = Database.MakeSafeForHtml(str);
					results.Add(string.Format(CultureInfo.CurrentCulture, "<font color={0}>{1}</font>", Database.HtmlColor(Item.GetColor(ItemStyle.Relic)), str));
				}
			}
			...
			if (itm.RelicInfo != null)
			{
				...
				if (r.Count > 0)
				{
					...
					string relicName = Database.MakeSafeForHtml(ToString(itm, false, true));
					...
					if (itm.Var1 < itm.RelicInfo.CompletedRelicLevel)
					{
						...
						str = Format(formatSpec, type, Math.Max(1, itm.Var1), itm.RelicInfo.CompletedRelicLevel);
					}
					else
					{
						string tag = "tagRelicComplete";
			...
			if (itm.Relic2Info != null)
			{
				...
				if (r.Count > 0)
				{
					...
					string relicName = Database.MakeSafeForHtml(ToString(itm, false, true, true));
					...
					if (itm.Var2 < itm.Relic2Info.CompletedRelicLevel)
					{
						...
						str = Format(formatSpec, type, Math.Max(1, itm.Var2), itm.Relic2Info.CompletedRelicLevel);
					}
					else
					{
						string tag = "tagRelicComplete";
			...
			string itemSeedString = Database.MakeSafeForHtml(string.Format(CultureInfo.CurrentCulture, Item.ItemSeed, itm.Seed, (itm.Seed != 0) ? (itm.Seed / (float)Int16.MaxValue) : 0.0f));
		...
		public static string GetItemSetString(Item itm)
		{
			...
			string[] setMembers = GetSetItems(itm, true);
			if (setMembers != null)
			{
		...
		private static void GetRequirementsFromRecord(SortedList<string, Variable> requirements, DBRecordCollection record)
		{
			...
			foreach (Variable variable in record)
			{
				...
				if (TQDebug.ItemDebugLevel > 1)
				{
					Log.Debug(Format("Added Requirement {0}={1}", key, value));
				}
		...
		private static string GetChanceModifier(int varNum, Variable modifierChanceVar)
		{
			...
			modifierChance = Format(formatSpec, modifierChanceVar[Math.Min(modifierChanceVar.NumberOfValues - 1, varNum)]);
			modifierChance = Database.MakeSafeForHtml(modifierChance);
			if (color != null)
			{
				modifierChance = Format("<font color={0}>{1}</font>", color, modifierChance);
			}
		...
		private static string GetDurationModifier(int varNum, Variable durationModifierVar)
		{
			...
			if (color != null)
			{
				durationModifier = Format("<font color={0}>{1}</font>", color, durationModifier);
			}
		...
		private static string GetModifier(ItemAttributesData data, int varNum, ItemAttributesData modifierData, Variable modifierVar)
		{
			...
			{
				modifier = Format("<font color={0}>{1}</font>", color, modifier);
			}
		...
		private static string GetChance(int varNum, Variable chanceVar)
		{
			...
			if (chanceVar != null)
			{
				chance = Format(formatSpec, chanceVar[Math.Min(chanceVar.NumberOfValues - 1, varNum)]);
				chance = Database.MakeSafeForHtml(chance);
				if (!string.IsNullOrEmpty(color))
				{
					chance = Format("<font color={0}>{1}</font>", color, chance);
				}
			}
		...
		private static string GetDamageRatio(int varNum, ItemAttributesData damageRatioData, Variable damageRatioVar)
		{
			...
			damageRatio = Format(formatSpec, damageRatioVar[Math.Min(damageRatioVar.NumberOfValues - 1, varNum)]);
			damageRatio = Database.MakeSafeForHtml(damageRatio);
			if (!string.IsNullOrEmpty(color))
			{
				damageRatio = Format("<font color={0}>{1}</font>", color, damageRatio);
			}
		...
		private static string GetDurationSingle(int varNum, Variable minDurVar, Variable maxDurVar)
		{
			...
			if (durationVariable != null)
			{
				duration = Format(formatSpec, durationVariable[Math.Min(durationVariable.NumberOfValues - 1, varNum)]);
				duration = Database.MakeSafeForHtml(duration);
				if (!string.IsNullOrEmpty(color))
				{
					duration = Format("<font color={0}>{1}</font>", color, duration);
				}
			}
		...
		private static string GetDurationRange(int varNum, Variable minDurVar, Variable maxDurVar)
		{
			...
			duration = Format(formatSpec, minDurVar[Math.Min(minDurVar.NumberOfValues - 1, varNum)], maxDurVar[Math.Min(maxDurVar.NumberOfValues - 1, varNum)]);
			duration = Database.MakeSafeForHtml(duration);
			if (!string.IsNullOrEmpty(color))
			{
				duration = Format("<font color={0}>{1}</font>", color, duration);
		...
		private static string GetAugmentSkillLevel(DBRecordCollection record, Variable variable, ItemAttributesData attributeData, string line, ref string font)
		{
			...
			if (!string.IsNullOrEmpty(skillRecordID))
			{
				...
				line = Format(formatSpec, variable[0], skillName);
			}
		...
		private static void ConvertOffenseAttributesToString(Item itm, DBRecordCollection record, List<Variable> attributeList, ItemAttributesData data, string recordId, List<string> results)
		{
			...
			if (!string.IsNullOrEmpty(label))
			{
				...
				{
					label = Format("<font color={0}>{1}</font>", labelColor, label);
				}
			}
			...
			if (!string.IsNullOrEmpty(modifier))
			{
				...
				modifierText = Format("<font color={0}>{1}</font>", Database.HtmlColor(Item.GetColor(ItemStyle.Epic)), modifierText);
				if (isGlobal)
				{
		...
		private static string GetAmountSingle(Item itm, ItemAttributesData data, int varNum, Variable minVar, Variable maxVar, ref string label, string labelColor, Variable minDurVar = null)
		{
			...
			if (currentVariable != null)
			{
				...
				amount = Format(formatSpec, currentVariable[Math.Min(currentVariable.NumberOfValues - 1, varNum)]);
				amount = Database.MakeSafeForHtml(amount);
				if (!string.IsNullOrEmpty(color))
				{
					amount = Format("<font color={0}>{1}</font>", color, amount);
				}
			}
		...
		private static string GetAmountRange(Item itm, ItemAttributesData data, int varNum, Variable minVar, Variable maxVar, ref string label, string labelColor, Variable minDurVar = null)
		{
			...
			amount = Format(formatSpec, min[Math.Min(min.NumberOfValues - 1, varNum)], max[Math.Min(max.NumberOfValues - 1, varNum)]);
			amount = Database.MakeSafeForHtml(amount);
			if (!string.IsNullOrEmpty(color))
			{
				amount = Format("<font color={0}>{1}</font>", color, amount);
		...
		private static int GetTriggeredSkillLevel(Item itm, DBRecordCollection record, string recordId, int varNum)
		{
		...
		private static int GetPetSkillLevel(Item itm, DBRecordCollection record, string recordId, int varNum)
		{
