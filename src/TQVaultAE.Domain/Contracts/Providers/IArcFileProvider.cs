...
namespace TQVaultAE.Domain.Contracts.Providers
{
	public interface IArcFileProvider
	{
		...
		byte[] GetData(ArcFile file, RecordId dataId);
		/// <summary>
		...
		/// <param name="recordId">Record we are writing</param>
		/// <param name="destinationFileName">Filename for the new file.</param>
		void Write(ArcFile file, string baseFolder, RecordId recordId, string destinationFileName);
	}
}