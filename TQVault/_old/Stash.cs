//-----------------------------------------------------------------------
// <copyright file="Stash.cs" company="bman654">
...
namespace TQVault
{
    ...
    public class Stash
    {
        ...
        /// <param name="suppressMessage">Indictes whether or not the Stash not found message is displayed</param>
        public void LoadFile(Database db, bool suppressMessage)
        {
        ...
        private void ParseRawData(Database db)
        {
            ...
            {
                int offset = 0;
                this.ParseItemBlock(db, offset, reader);
                ...
                try
                {
                    ...
                    using (StreamWriter outStream = new StreamWriter(outfile, false))
                    {
                        ...
                        if (this.sack.NumberOfItems > 0)
                        {
                            outStream.WriteLine();
                            outStream.WriteLine("SACK 0");

                            for (int j = 0; j < this.sack.NumberOfItems; ++j)
                            {
                                Item item = this.sack.GetItem(j);
        ...
        private void ParseItemBlock(Database db, int fileOffset, BinaryReader reader)
        {
